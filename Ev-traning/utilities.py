...
def mouse_drag(start_x, start_y, target_x, target_y):
    pyautogui.moveTo(start_x, start_y)  # Move to the top-left corner
    pyautogui.mouseDown()  # Press the mouse button down
    pyautogui.moveTo(target_x, target_y, duration=0.1)  # Drag to the target location
    pyautogui.mouseUp()


def click_image(image_path, offset_x=0, offset_y=0, wait=0):
    ...
    for i in range(len(path_grid)):
        for n in range(int(path_grid[i][1])):
            if path_grid[i][0] == "right":
                ...
                time.sleep(0.05)
                pyautogui.keyUp("D")
            elif path_grid[i][0] == "left":
                pyautogui.keyDown("A")
                time.sleep(0.05)
                pyautogui.keyUp("A")
            elif path_grid[i][0] == "down":
                pyautogui.keyDown("S")
                time.sleep(0.05)
                pyautogui.keyUp("S")
            elif path_grid[i][0] == "up":
                pyautogui.keyDown("W")
                time.sleep(0.05)
                pyautogui.keyUp("W")


def recognize_text(image_path, text_score=0.9):
    """Recognize text from the image using EasyOCR."""
    result = reader.readtext(image_path, detail=1, paragraph=False)
    if result:
        return result[0][1]  # Combine all recognized texts into one string
    return None, None  # Handle the case where result is None
